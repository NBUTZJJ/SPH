        ...
        >
          <li class="cart-list-con1">
            <input
              ...
              @change="changeCheck(cartInfo, $event)"
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input
          class="chooseAll"
          type="checkbox"
          :checked="isAllChecked == 1 && count>0"
          @change="AllCheck"
...
export default {
  ...
  methods: {
    ...
    async changeCheck(cartInfo, $event) {
      try {
  ...
  async  AllCheck(event) {
    try {
        let checked=event.target.checked ? "1" : "0";
        await this.$store.dispatch("allCheck",checked)
        this.getData()
      } catch (error) {
        alert(error.message)
      }
    },
    handler: _.throttle(async function (type, value, cartInfo) {
      switch (type) {
        case "plus":
          value = 1;
          break;
    ...
    async deleteShopCart(cartInfo) {
      try {
        await this.$store.dispatch("delCartList", cartInfo.skuId);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
